...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        List<User> usersScoreOrderWise;
        if(scoreOrder==ScoreOrder.ASC){
            usersScoreOrderWise= userRepository.findAll().stream().sorted(Comparator.comparingInt(User::getScore)).collect(Collectors.toList());
        }
        else{
            usersScoreOrderWise= userRepository.findAll().stream().sorted(Comparator.comparingInt(User::getScore).reversed()).collect(Collectors.toList());
        }
     return usersScoreOrderWise;
    }
